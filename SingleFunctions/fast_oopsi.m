...
if V.fast_iter_max>1;
    ...
    if ~isfield(V,'est_b'),     V.est_b     = 0; end    % whether to estimate b
...
if ~isfield(P,'gam'),   P.gam   = (1-V.dt/3)*ones(V.Ncells,1);  end %tau=dt/(1-gam)
if ~isfield(P,'lam'),   P.lam   = .1*ones(V.Ncells,1);          end
